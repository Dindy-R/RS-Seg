...
from datasets.ImgMaskDataset import OwnDataset, DualDataset, get_val_transform
...
def run(cfg_name, weight):
    ...
    with open(os.path.join(cfgD['data_dirs'], "ImageSets/test.txt"), "r") as f:
        test_lines = f.readlines()
    # test_data = DualDataset(data_dirs=cfgD['test_dirs'], transform=val_transform)
    test_data = OwnDataset(test_lines, False, cfgD['data_dirs'], transform=val_transform)
...
def get_args():
    ...
    argparser.add_argument('-c', '--config', default='segformer', type=str,  help='Name of the config file.')
    argparser.add_argument('-w', '--weight', default='best.pth', type=str, help='Choice a weight file.')
