...
import torch
from .resnet import ResNet, Bottleneck, ResNet_Botnet
...
def resnest50(pretrained=None, root='~/.encoding/models', **kwargs):
...
@RESNEST_MODELS_REGISTRY.register()
def resnest101(pretrained=None, root='~/.encoding/models', **kwargs):
    model = ResNet_Botnet(fmap_size=(14, 14), block=Bottleneck, layers=[3, 4, 23, 3],
                   radix=2, groups=1, bottleneck_width=64,
                   deep_stem=True, stem_width=64, avg_down=True,
                   avd=True, avd_first=False, botnet=True, **kwargs)
    if pretrained == 'imagenet':
        model.load_state_dict(torch.hub.load_state_dict_from_url(
